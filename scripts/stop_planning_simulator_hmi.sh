...
close_hmi_windows() {
  if command -v xdotool >/dev/null 2>&1; then
    ...
    xdotool search --name 'APS HMI Frontend' 2>/dev/null | xargs -r -n1 xdotool windowclose >/dev/null 2>&1 || true
...
stop_pid_file "${LAYOUT_PID_FILE}" "${LAYOUT_PGID_FILE}" "HMI layout helper"
stop_pid_file "${RVIZ_PID_FILE}" "${RVIZ_PGID_FILE}" "RViz user"
stop_pid_file "${FRONTEND_PID_FILE}" "${FRONTEND_PGID_FILE}" "HMI frontend"
stop_pid_file "${HMI_PID_FILE}" "${HMI_PGID_FILE}" "HMI container"
stop_pid_file "${AUTOWARE_PID_FILE}" "${AUTOWARE_PGID_FILE}" "planning_simulator"
stop_pattern_group 'aps_hmi_container/lib/aps_hmi_container/aps_hmi_container .*:: frontend' 'HMI frontend'
stop_pattern_group 'rviz2 .*:: rviz' 'RViz user'
stop_pattern_group 'aps_hmi_container/lib/aps_hmi_container/aps_hmi_container .*--frontend-only true' 'HMI frontend'
stop_pattern_group 'rviz2 .* -qwindowtitle RViz User' 'RViz user'
stop_pattern_group 'hmi_compose_layout.sh .*aps hmi frontend' 'HMI layout helper'
stop_pattern_group 'aps_hmi_container/lib/aps_hmi_container/aps_hmi_container' 'HMI container'
stop_pattern_group '/opt/ros/.*/bin/ros2 launch autoware_launch planning_simulator.launch.xml' 'planning_simulator launcher'
close_hmi_windows
